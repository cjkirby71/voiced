...
export function AuthProvider({ children }) {
  ...
  const signInWithMagicLink = async (email) => {
    try {
      ...
      const errorMsg = "Something went wrong sending magic link. Please try again.";
      setAuthError(errorMsg);
  ...
  // Exchange code for session (for magic link callbacks)
  const exchangeCodeForSession = async (code) => {
    try {
      ...
      if (!result?.success) {
        setAuthError(result?.error || "Failed to process magic link");
        return { success: false, error: result?.error };
      }

      return { success: true, data: result.data };
    } catch (error) {
      const errorMsg = "Something went wrong processing your magic link. Please try again.";
      setAuthError(errorMsg);
